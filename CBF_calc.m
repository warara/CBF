...
%     F =@(v)(-2*[F_r(v)/para.m^2; 0]);
    ...
    y = zeros(para.num_Agents, length(t_), 1);
    y(:,1) = y0;
...
%     h_it = zeros(length(t_), 2);
    ...
    for i=2:length(t_)  
         z = [y(1,i-1)-y(2,i-1); y(2,i-1)-y(1,i-1);y(2,i-1)-y(3,i-1);y(3,i-1)-y(2,i-1);];
%          A = [(1/(h(z(1)))^2-1/(1+h(z(1)))^2)*z(1)/norm(z(1)), -(1/(h(z(1)))^2-1/(1+h(z(1)))^2)*z(1)/norm(z(1)) 0;
%               (1/(h(z(2)))^2-1/(1+h(z(2)))^2)*z(2)/norm(z(2)), -(1/(h(z(2)))^2-1/(1+h(z(2)))^2)*z(2)/norm(z(2)) 0;
%               0, (1/(h(z(3)))^2-1/(1+h(z(3)))^2)*z(3)/norm(z(3)), -(1/(h(z(3)))^2-1/(1+h(z(3)))^2)*z(3)/norm(z(3));
%               0, (1/(h(z(4)))^2-1/(1+h(z(4)))^2)*z(4)/norm(z(4)), -(1/(h(z(4)))^2-1/(1+h(z(4)))^2)*z(4)/norm(z(4))];
%          
         
         
         
        A =   [z(1)/norm(z(1)),-z(1)/norm(z(1)), 0;
               z(2)/norm(z(2)),-z(2)/norm(z(2)), 0;
               0, z(3)/norm(z(3)),-z(3)/norm(z(3));
               0, z(4)/norm(z(4)),-z(4)/norm(z(4));];
        A_ = [1 0 0;
             -1 0 0;
              0 1 0;
              0 -1 0;
        ...
        b =  [ para.gamma*(norm(z(1))-para.d)^3;  
               para.gamma*(norm(z(2))-para.d)^3;
               para.gamma*(norm(z(3))-para.d)^3;
               para.gamma*(norm(z(4))-para.d)^3;];
           
%         b = para.gamma*[-1/log(h(z(1))/(1+h(z(1))));
%             -1/log(h(z(2))/(1+h(z(2))));
%             -1/log(h(z(3))/(1+h(z(3))));
%             -1/log(h(z(4))/(1+h(z(4))))];
        b_ = [8; 8; 8; 8; 8; 8];
        ...
        %[u_zw, ~, exitflag] = fmincon(@(u)(u-ud)'*H*(u-ud),[ 0;0;0], A, b, [], [], [], [], [], options_quad);
        %zw = quadprog(H, 0, A, b-A*ud);
        u=quadprog(H, [0 0 0]', A, b-A*ud,[], [], [], [],[0 0 0], options_quad)+ud;
...
%             l = 1;         
%             for k = [iif(j==1, [],j-1) iif(j==para.num_Agents, [], j+1)]
%                 if(j~=k)
%                     z = y(k,i-1)-y(j,i-1);
%                     A(l) =   -z/norm(z);
%                     b(l) =   para.gamma*(norm(z)-para.d)^3;
% %                     A(l) =   A_cbf (y(j,i-1), (y(j,i-1)-y(k,i-1)), 0);
% %                     b(l) =   b_cbf (y(j,i-1), (y(j,i-1)-y(k,i-1)), 0);
...
% %             [z, ~, exitflag] = quadprog(H, [0], A, b-A*ud(j), [], [], [], [], 0, options_quad);
% %             u(j) = z + ud(j);
...
function plot_states(t, y, para)
    ...
    plot(t, y)
    subplot(2,1,2);
    plot(t,  [zeros(1,length(t)); (abs(y(1,:,:,1))+abs(y(2,:,:,1)))-para.d;...
